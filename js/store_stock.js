...
$(document).ready(function () {
    ...
    $("#section_stock_tbody").on("keydown", "span", function (e) {
        ...
        if (e.key === "Enter") {
            ...
            insert_jaysan_stock1(part, godown, department, section, qty, '{}');
    ...
    $("#req_btn").on("click", function () {
        ...
        insert_emp_material_request1(dep_id, part_id, qtyy);
    });


    $("#section_min_max_view").on("change", function () {
        get_jaysan_stock1(sec_id, dep_id, godown_id);
    })
    // get_jaysan_stock(27)


    get_jaysan_stock1(sec_id, dep_id, godown_id);
    get_requset_report1(dep_id, 'dep');
    ...
    $("#section_stock_tbody").on("keydown", ".qty", function (e) {
        ...
        if (e.key === "Enter") {
            ...
            insert_jaysan_stock1(part_id, godown_id, dep_id, sec_id, qty, '{}');
    ...
    $("#sec_confirm_btn").on("click", function () {
        ...
        update_stock_allocation_section1(allocated_id, qty, current_user_id, remark);
    })
...
function update_stock_allocation_section1(allocation_id, received_qty, received_by, receive_remark) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.trim() == "ok") {
                ...
                get_jaysan_stock1(sec_id, dep_id, godown_id);
                get_requset_report1(sec_id, 'dep');
            }
...
function get_requset_report1(store_id, store_type) {
...
function insert_jaysan_stock1(part, godown_id, dep_id, sec_id, qty, stock_master_json) {
...
function get_jaysan_stock1(sec_query, dep_query, creditor_query) {
                            ...
                            (unitObj.department_details || []).forEach(function (depObj, depIndex) {
                                ...
                                sections.forEach(function (secObj, secIndex) {
                                    ...
                                    if (secIndex === 0) {
                                        ...
                                        var d_min_max = '';
                                        var remaining_qut = 0;
                                        remaining_qut = parseFloat(depObj.dep_max) - parseFloat(depObj.department_qty);
                                        ...
                                        tr += `<td rowspan="${itemRowSpan}">${count}</td>`;
                                        tr += `<td rowspan="${itemRowSpan}"> ${item.part_name || ""}</td>`;
                                        tr += `<td rowspan="${sections.length}">${dd_req} ${depName} - <span contenteditable class="border border-primary p-1 me-2 border-2 rounded-3"  data-stock_id='${item.stock_id}' data-part_id='${item.part_id}' data-unit_id='${unitObj.godown_id}' data-dep_id='${depObj.dep_id}'>${depObj.department_qty}</span> ${d_min_max}</td>`;
                                    }
                                    ...
                                    if ((depObj.department_qty <= depObj.dep_min || depObj.department_qty <= depObj.dep_max)) {

                                        if (Array.isArray(depObj.dep_req) && depObj.dep_req.length > 0) {

                                            depObj.dep_req.forEach(function (sts) {
...
function insert_emp_material_request1(store_id, part_id, qtyy) {
