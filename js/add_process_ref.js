...
$('#final_output_btn_grp').removeClass("d-flex").addClass("d-none");
 ...
 $('#out_breadcrumb').empty()
  ...
  $('#submit_btn').on('click', function() {
    ...
    var input_count =  $('#input_part_tbl tr').length;
...
var output_part = $('#output_part_txt').data('out-part-id')
 ...
 if(input_count>=1 && process_count>=1 && output_part !="")
 {
  ...
  var processData = []
var inputPartsData = [];
$('#input_part_tbl tr').each(function(index) {
...
$("#process_tbl").sortable({
  ...
  update: function(event, ui) {
    $('#process_tbl .card').each(function(index) {
      console.log(index);
      console.log($(this).data('process-id'));
    });
  }
}).disableSelection();
  ...
  if($('#part_no').val() && $('#part_name').val() && $('#qty').val() &&  $('#part_name').data('selected-part_id') )
    {
      ...
      var processId = $('#finished_process').data('process-id');
  var pre_process = "<p class='text-danger h6'> No Previous process</p>"
  console.log(partId);
  if($('#finished_process').text().trim()!== '')
    pre_process = $('#finished_process').html()
  
    var trlength = $('#input_part_tbl tr').length+1;
  $('#input_part_tbl').append(" <tr data-part-id="+partId+" data-part-qty="+$('#qty').val()+"  data-pre-process-id="+processId+"> <td  class='sno'>"+trlength+"</td> <td >"+ $('#part_no').val()+"-"+  $('#part_name').val()+",Qty : "+ $('#qty').val()+"<br>"+ pre_process + "</td> <td class='text-danger h6'><button class='btn btn-outline-danger border-0' type='button'> <i class='fa fa-trash' aria-hidden='true'></i></button> </td> </tr>")
...
$('#process_tbl').append($('#imp_process').html())
});
...
swal({
  ...
  if (isConfirm) {
    thisrow.closest('tr').fadeOut(500, function() {
      $(this).remove();
      $('#input_part_tbl tr').each(function(index) {
        console.log( $(this).find('.sno').text());
        $(this).find('.sno').text(index + 1);
      });
      console.log(thisrow.closest('tr').data('part-id'))
  });
...
count = count +1;
$('#imp_process').append(" <div class='card mt-1' data-process="+obj.process+ " data-id="+obj.process_id+ "> <div class='card-header'> <h6>"+obj.process_name+"</h6> </div> </div>")
    });
...
count = count +1;
$('#imp_input_part').append(" <div class='card av_nxt mt-1' data-pre-process-id="+obj.previous_process_id + " data-part-id="+obj.output_part  +" data-id="+ obj.id + " data-part-no="+obj.part_no + " data-part-name="+obj.part_name+  " data-process="+obj.process_name+ "> <div class='card-header d-flex justify-content-center'>"+ obj.part_name + "-"+ obj.part_no + "</div> <div class='card-body'> <i class='fa-solid fa-screwdriver-wrench pe-2'></i>"+ obj.process_name +"</div></div>")
...
count = count +1;
$('#imp_output_part_txt').data('out-part-id', obj.part_id);
...
var count =0 
  ...
  obj.forEach(function (obj) {
     count = count +1;
$('#pa_nxt_prs').append("<div class='card av_nxt' data-process-id="+obj.process_id + " data-part-id="+obj.output_part  + " data-part-no='"+obj.part_no + "' data-part-name='"+obj.part_name+  "' data-process='"+obj.process_name+ "'> <div class='card-header d-flex justify-content-center'>"+ obj.part_name + "-"+ obj.part_no + "</div> <div class='card-body'> <i class='fa-solid fa-screwdriver-wrench pe-2'></i>"+ obj.process_name +"</div></div>")
