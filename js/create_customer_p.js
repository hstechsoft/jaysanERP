...
$("#create_work_btn").click(function()
   {
    ...
    if($('#assigntoothers_checkbox').is(":checked"))
    {
      ...
      $("#work_select_employee_table tr:gt(0)").each(function () {
        ...
        var emp_id_p = $.trim(this_row.find('td:eq(1)').html())
        var emp_name_p = $.trim(this_row.find('td:eq(2)').html())
...
$("#cus_lead_source1").change(function(){
  if ($('#cus_lead_source1').find(":selected").text() == "Other")
  {
    $("#cus_lead_source").removeAttr("disabled");
    $("#cus_lead_source").val("")
    $("#cus_lead_source").focus()
  }
  else{
    $("#cus_lead_source").val( $('#cus_lead_source1').find(":selected").text()) 
  }
...
{
    var url = "create_work.html?cus_id=" + cus_id + "&phone_id=" + phone_id;
