...
type_p = "pending"
  ...
  {
   window.location.href =  "admin_emp_single_detail.html?phone_id=" +phone_id  +"&emp_id=" + emp_id
  });
...
$('#sel_usr_in').change(function() {
  ...
  $("#selected_usr_txt").text( $('#sel_usr_in :selected').text())
  if(current_tab == "request-tab")
    get_employee_tour_pending()
    else
  get_employee_tour()
...
update_att_req(req_id,"approved")
  })
  $(document).on('click', '.decline_btn', function() {

    var req_id = $(this).data("id")
    update_att_req(req_id,"declined")
      })

      $(document).on('click', '.skip_btn', function() {

        var req_id = $(this).data("id")
        update_att_req(req_id,"skipped")
          })
...
get_task(emp_id,$(this).data('from_date'),$(this).data('to_date'))
...
return formattedDate
  ...
  $('#menu_bar').hide()
   ...
   function update_att_req(req_id,sts)
   {



$.ajax({
  url: "php/update_att_req.php",
...
var des = ""
var d =""
if(type_p == "approved" || type_p == "declined")
  d= "d-none "
var count =0 
  obj.forEach(function (obj) {
  des =  "leave approval for "+ obj.reason +" from <span class = 'text-danger'>"+obj.from_date+" to "+obj.to_date + "</span>"
    $("#req_data").append(" <div class='list-group-item list-group-item-action flex-column '> <div class='d-flex justify-content-between'> <div><h6>"+obj.emp_name+"</h6></div><div><p class='text-muted'>"+obj.dated+"</p></div> </div> <p class='mb-1'>"+obj.emp_name+"<span class='text-muted'> "+ des + "</span></p> <div class='d-flex justify-content-between align-items-center shadow-sm '> <div class=''> <p class='text-secondary mb-0'> Planned Works </p> </div> <div> <button class='btn btn-light show_planned_works'  data-from_date='"+obj.from_date+"'  data-to_date='"+obj.to_date+"'><i class='fa-solid fa-caret-down' aria-hidden='true'></i></button> </div> </div> <div class='"+d+" d-flex justify-content-between mt-1 shadow'> <button class='btn btn-warning btn-sm decline_btn'  data-id='"+obj.id+"' >Decline</button><button class='btn btn-success btn-sm approve_btn'  data-id='"+obj.id+"'>Approve</button><button class='btn btn-secondary btn-sm skip_btn'   data-id='"+obj.id+"'>Skip</button> </div> <div class='list-group planned_works shadow-sm mt-1 ' style='max-height: 75vh; overflow-y: auto;'> </div> </div>")
  ...
  error: function (xhr) {
   ...
   function get_employee_tour_pending()
   {

  

$.ajax({
  url: "php/get_admin_tour_requset.php",
  type: "get", //send it through get method
  data: {
