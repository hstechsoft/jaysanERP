...
var emp_query = 1
var work_type_query = 1

get_assigned_work_all()
 ...
 $("#download_xl_btn").on('click',function()
{
  $("#remind_table1").table2excel({
             ...
             exclude: ".noExl",
							...
							filename: "Assigned_work_report_" + new Date().toLocaleDateString().replace(/[\-\:\.]/g, "_") + ".xls",
							fileext: ".xls",
							exclude_img: true,
							exclude_links: true,
							exclude_inputs: true,
...
get_assigned_work_all()          
 ...
 $("#report_search_btn").click(function()
 {
 
  {
    ...
    date_query_end = "'"+  get_millis(get_millis($('#search_start_date').val()))+ "'"
   
    get_assigned_work_all() 
    report_date = $('#search_start_date').val() + " - " + $('#search_start_date').val()
 }
      ...
      if( $('#sel_usr_in').find(":selected").val()>0)
    {
     ...
     $('#sel_usr_in').change(function() {
      ...
      if ($(this).find(":selected").val()!="0")
      {
        ...
        emp_query = "work.emp_id" + " = " + $(this).find(":selected").val()
...
$('#work_type_list').on('change', function() {
  ...
  {
    // get_work($(this).find(":selected").val());
    work_type_query = "work.work_type" + " = " + "'"+$(this).find(":selected").text() + "'"
...
$('#lead_modal').modal('hide'); 
    ...
    if($("#cus_need").find(":selected").text() =="restamping")
    {
      ...
      ",stamp_certificate - " +  $('#stamp_certificate').val() + 
      ",stamp_plate - " +  $('#stamp_plate').val() + 
   ...
   function get_assigned_work_all()
   {
...
$.ajax({
  ...
  data: {
    ...
    ass_emp_id : emp_query,
    ass_work_type : work_type_query
},   
...
beforeSend: function() {
  ...
  success: function (response) {
    ...
    $('#report_info').html("Employee  - " +   $('#sel_usr_in').find(":selected").text() + "<br> Work Type - " +$('#work_type_list').find(":selected").text() + "<br> Date Range - " + report_date)
    $('#loader_dis').hide()

$("#pendig_work_table").empty()
...
var count =0 
  obj.forEach(function (obj) {
   ...
   count  = count + 1;
    ...
    $("#pendig_work_table").append(" <tr> <td>"+ count + "</td> <td>"+ obj.cus + "</td> <td>"+ obj.emp_name + "</td><td>"+ obj.his + "</td>  <td>" +obj.ct+ "</td></tr>")
...
if(obj.cus_location != "")
{
  ...
  if(obj.cus_location.search("http") != -1)
  {
   ...
   function get_sa_work_all()
   {
    
   $.ajax({
     url: "php/get_sa_work_all.php",
     type: "get", //send it through get method
     data: {
       emp_id: current_user_id
