...
$(document).ready(function(){
 ...
 get_quote_details()
  ...
  $("#invoice_conv").attr("onclick"," window.location.href = 'proforma_invoice.html?cus_id=" + cus_id+ "&conv_type=quotation&conv_no=" + qno+ "';")
...
console.log(des)
   ...
   function get_quote_details()
   {
    ...
    $.ajax({
        url: "php/get_quote_details_single.php",
        type: "get", //send it through get method
        data: {
            qno : qno
      ...
      if (response.trim() != "error") {
        ...
        if((response.trim() != "0 result"))
        {
          ...
          obj.forEach(function (obj) {
            ...
            $("#print_qno").text(obj.qnos) 
          
        
            $("#print_bank").html(obj.company_bank.replace(/\n/g, "<br />"))
            $("#print_terms").html((obj.company_terms_q).replace(/\n/g, "<br />"))
...
total_value = obj.total_value
   ...
   function get_quotion_single()
   {
    ...
    $.ajax({
        url: "php/get_quotation_single.php",
        type: "get", //send it through get method
        data: {
            qno : qno
