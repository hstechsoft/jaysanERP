  ...
  $("#cus_table").on("click", "tr td button", function (event) {
...

    {
      swal({
        ...
        text: "You will not be recover this customer again!",
        icon: "warning",
        buttons: [
          'No, cancel it!',
          'Yes, I am sure!'
        ...
        if (isConfirm) {
          swal({
            ...
            delete_customer(product_id) // <--- submit form programmatically
...
function get_all_customer_excel() {
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (response.trim() === "error") {
        salert("Error", "Unable to fetch customers", "error");
        return;
      }
      ...
      // Excel header row
      let csv = "Customer Name,Phone,Address,Customer Type,GST No\n";

      data.forEach((obj) => {
        csv += `"${obj.cus_name}","${obj.cus_phone}","${obj.cus_address}","${obj.sub_group_name || ''}","${obj.cus_gst}"\n`;
      });

      const blob = new Blob([csv], { type: "text/csv;charset=utf-8;" });

      const today = new Date().toISOString().split("T")[0];
      const filename = `deal_${today}.csv`;
