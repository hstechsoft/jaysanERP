...
$.ajax({
  url: "php/get_today_call_incoming.php",
  type: "get", //send it through get method
  data: {
   phone_no : phone_no
...
if (response.trim() != "error") {
  ...
  if(response.trim() !="0 result")
  {
    ...
    obj.forEach(function (obj) {
      ...
      $("#site_work_table").append("  <tr><td > " + count + " </td><td class='text-nowrap' >" + millis_to_date(parseFloat(obj.work_date)) + "</td><td >" + obj.work_type  + " - " + obj.work_description + "</td></tr>")
  ...
  error: function (xhr) {
   ...
   function update_work_sql()
   {
    ...
    if($("#status_in").val() == 2 || $("#status_in").val() == 4 || $("#status_in").val() == 5)   {
       ...
       status = "update status  - " + $("#status_in :selected").text()  + " - "+ $("#remarks_in").val() + " so work is reschudled  to " + millis_to_date(get_millis($("#sdate_in").val())) + " by " + current_user_name +  " on " + get_today_date();
     work_date =  get_millis($("#sdate_in").val());
     work_com_status = "incomplete"
    }
    else
    {
      status= "update status  - " + $("#status_in :selected").text() + " - "+ $("#remarks_in").val()  + " so work Finished by " + current_user_name +  " on " + get_today_date();
