...
$(document).ready(function(){
 ...
 $("#quotation_conv").attr("onclick"," window.location.href = 'quotation.html?cus_id=" + cus_id+ "&conv_type=estimation&conv_no=" + qno+ "';")

 $("#invoice_conv").attr("onclick"," window.location.href = 'sales.html?cus_id=" + cus_id+ "&conv_type=estimation&conv_no=" + qno+ "';")
 ...
 get_quote_details()
  ...
  // $("#tlead_att_on").text(localStorage.getItem("ls_uname"))
   ...
   function get_quote_details()
   {
    ...
    $.ajax({
        url: "php/get_estimate_details_single.php",
...
total_value = obj.total_value
   ...
   function get_quotion_single()
   {
    ...
    $.ajax({
        url: "php/get_estimation_single.php",
        type: "get", //send it through get method
        data: {
            qno : qno
      ...
      if (response.trim() != "error") {
        ...
        if((response.trim() != "0 result"))
        {
          ...
          obj.forEach(function (obj) {
            ...
            $("#print_pro_table").append("<tr><td colspan='7'><div class='d-flex justify-content-start gap-4'><img  class='img-fluid' style='width: 70px;' src='" + obj.image1_addr + "' alt> <img class='img-fluid' style='width: 70px;' src='" + obj.image2_addr + "' alt> <img class='img-fluid' style='width: 70px;' src='" + obj.image3_addr + "' alt></div></td></tr>")
          });
        
    //       $("#print_pro_table").append("<tr> <th colspan='5' class='text-center'>" + "Total" + "</th> <th class='text-center'>" + "0" + "</th> <th class='text-end text-nowrap'  >&#x20B9 " +(parseInt(total_value) - parseInt(shipping) ).toFixed(2) + "</th> </tr>")
...
get_current_userid_byphoneid();
  ...
  //  get today 
   ...
   function conv_html(txt)
   {
    ...
    var con_result = ""
    txt =txt.replace(/\n/g, "<br />")
    $("#mysql_plain_txt").html(txt)
    $("#mysql_conv_txt").append( "<p> "+   $("#mysql_plain_txt").val() + "</p>")
