...
$(document).ready(function () {
  ...
  $('#order_list').on('click', '.pro_btn', function () {
    ...
    if (qty > 0) {
      console.log($(this).val(), qty, selected_type, selected_date);
      if (selected_type == 'Production' && selected_date) {
        insert_assign_product($(this).val(), qty, selected_type, selected_date)
      }
      else if (selected_type == "Finshed") {
        if ($("#godown").val() != undefined) {
          insert_assign_product($(this).val(), qty, selected_type, selected_date)
        }
        else {
          shw_toast("Error", "Please Select The Godown/Unit", "error")
        }
      }
      else if (selected_type == "Waiting") {
        insert_assign_product($(this).val(), qty, selected_type, selected_date)
      }
      else {
        shw_toast("Error", "Please Select The Date", "error")
      }
