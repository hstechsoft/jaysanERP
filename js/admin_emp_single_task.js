...
get_task()
      console.log(format_date_mysql(date_end));
        ...
        datesSet: function(info) {
         ...
         get_calender_task(format_date_mysql(format_date_start(csdate)),format_date_mysql(format_date_start(cedate)))
        }
...
$(document).ready(function(){
  ...
  {
   window.location.href =  "admin_emp_single_detail.html?phone_id=" +phone_id +"&emp_id=" +sel_emp_id 
  });
...
$("#works").on('click', '.show_history', function() {
...
displayCurrentWeek()
   ...
   function get_calender_task(sdate,edate)
   {

 
$.ajax({
  url: "php/get_emp_task_report_cal.php",
...
$.ajax({
  ...
  emp_id : sel_emp_id,
  start_date :format_date_mysql(date_start) ,
  end_date : format_date_mysql(date_end)
},
  success: function (response) {
console.log(response);
...
{
  $("#works").append(" <div class='text-bg-secondary p-3 d-flex justify-content-center'><h6 class=''>No Work</h6> </div>")
}
