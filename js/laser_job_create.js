...
$("#qty_in").hide()
$("#job_no_in").hide()
$("#shift_change_drop").hide()

$('#job_change_drop').change(function() {
    //$('# qty').find(':selected').text()
    //$('# qty').find(':selected').val()
    if($('#job_change_drop').find(':selected').val() == 3)
    { $("#job_no_in").show()
        $("#qty_in").hide()
        $("#shift_change_drop").hide()
    }
       
    else if($('#job_change_drop').find(':selected').val() == 4)
    {
        $("#job_no_in").show()
        $("#qty_in").show()
        $("#shift_change_drop").show()
    }
    else if($('#job_change_drop').find(':selected').val() == 5)
        {
            $("#job_no_in").hide()
            $("#qty_in").hide()
            $("#shift_change_drop").show()
...
$('#add_nest_btn').on('click', function()
{
...
$("#job_card_list").on("click","tr td button", function(event) {
 
    if($(this).closest("td").index() == 14)
    {
        $("#modify_job_modal1").modal('show');
    }
