...
WHERE bom_output.bom_id in(SELECT bom_id FROM bom_output WHERE component_cat <> 'Process' AND component_cat <> 'Porcess') 
UNION ALL
SELECT 
...
INNER JOIN parts_tbl in_part_child
   ...
   SELECT *
FROM (
    SELECT  output_part_id,input_part_id,qty as qty, sum(qty) over (PARTITION BY input_part_id) as total_qty,output_part_name,input_part_name,level,part_path,sub_ass,
