...
stock_wise as (SELECT process_available.process_available_id, process_available.qty*22 as required_qty,ifnull(sum(js.qty),0) as available_qty, if(ifnull(sum(js.qty),0) < (process_available.qty*22), true, FALSE) as is_not_available, (process_available.qty*22) - ifnull(sum(js.qty),0) as shortage_qty FROM process_available 
...
GROUP BY process_available.process_available_id),
  bom_plan as (
        ...
        FROM
          ...
          WHERE iwp_parent.process_id = 2796
