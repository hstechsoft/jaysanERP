...
FROM qr_work_entry
    ...
    WHERE
   qr_work_entry.production_id in (SELECT production_id FROM qr_work_entry INNER join work_process on qr_work_entry.qr_work_id = work_process.current_work_id WHERE qr_work_entry.work_sts = 'finished' and production_id > 0 GROUP BY production_id) group by qr_work_entry.qr_work_id),
...
inner join work_process on production_summary1.qr_work_id = work_process.current_work_id
    ...
    LEFT join parts_tbl on work_process.part_id = parts_tbl.part_id GROUP BY production_id
