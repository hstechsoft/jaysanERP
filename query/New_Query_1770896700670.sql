...
with RECURSIVE bom_hi as(SELECT
   ...
   ifnull(bom_correction.bomlist_id,(SELECT bolist.bom_id FROM bom_output bolist WHERE bolist.part_id = bom_input.part_id and bolist.component_cat <> "Process" LIMIT 1)) as bomlist_id,
...
SELECT 
 bom_output_child.bom_id AS parent_bom_id,
    ...
    ifnull(bom_correction_child.bomlist_id,(SELECT bolist.bom_id FROM bom_output bolist WHERE bolist.part_id = bom_input_child.part_id and bolist.component_cat <> "Process" LIMIT 1)) as bomlist_id,
    ...
    IF(
       ...
       (SELECT JSON_ARRAYAGG(JSON_OBJECT('bom_id',bo1.bom_id,'component_cat',bo1.component_cat)) from bom_output bo1 WHERE bo1.part_id = bom_input_child.part_id and bo1.component_cat <> "Process" GROUP BY bo1.part_id ) as bom_list,
       level +1 as level
...
LEFT JOIN bom_correction bom_correction_child
   ...
   AND bom_correction_child.outpart_bom_id = 10 
