with summary_all as(SELECT subgroup.sub_group_name,cgroup.group_name,mtype.type_name,model.model_name,pro.product_name,pro.product_id,model.model_id,mtype.mtid,cgroup.group_id,subgroup.sub_group_id FROM subgroup_type_price sub_price
...
GROUP BY pro.product_id,model.model_id,mtype.mtid,subgroup.sub_group_id ORDER BY subgroup.sub_group_name,cgroup.group_name,mtype.type_name,model.model_name,pro.product_name),

sub_group_summary as(SELECT JSON_ARRAY(JSON_OBJECT('sub_group_name',sub_group_name,'sub_group_id',sub_group_id)) as sub_group_details,group_name,type_name,model_name,product_name,product_id,model_id,mtid,group_id FROM summary_all GROUP BY product_id,model_id,mtid,group_id ORDER BY sub_group_name,group_name,type_name,model_name,product_name)
