...
SELECT bom_hi.*,if(input_part=output_part,'duplicate_entry','') as duplicate, ROW_NUMBER() OVER () as row_num,(SELECT bom_output.bom_id from bom_output WHERE bom_output.part_id = bom_hi.input_part and component_cat = bom_hi.component_cat) as bom_check FROM bom_hi 
