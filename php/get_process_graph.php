...
WITH RECURSIVE process_details AS (
   ...
   process_final as(SELECT pd.*,ip.input_part_id,(SELECT parts_tbl.part_name from parts_tbl WHERE parts_tbl.part_id = ip.input_part_id ) as part_name ,
      JSON_ARRAYAGG(
        JSON_OBJECT(
          'process_availble',
            (SELECT count(process_wel_tbl.process_id) FROM process_wel_tbl WHERE process_wel_tbl.output_part =  ip.input_part_id  and cat = "out" group by process_wel_tbl.output_part) ,
            'previous_process_id',
   ...
   from process_details pd
    ...
    stock_info as(select pf.*, srv.part_id ,  JSON_ARRAYAGG(
        JSON_OBJECT( 'godown', srv.godown, 'dep', srv.dep, 'sec', srv.sec, 'stock_id', srv.stock_id, 'creditor_name', pf.output_part, 'dep_name', srv.dep_name, 'sec_name', srv.sec_name,'qty', srv.qty, 'reserve_qty', srv.reserve_qty, 'available_qty', srv.available_qty, 'reserve_details', srv.reserve_details) ) as stock_details,sum(srv.available_qty) as total_available_qty,sum(srv.reserve_qty) as total_reserve_qty,sum(srv.qty) as total_qty from process_final pf   

        left join stock_reserve_view srv on CASE
       WHEN pf.output_part IS NOT NULL
           THEN srv.part_id = pf.output_part 
       ELSE
           srv.process_id = pf.process_id 
   END
    -- left join stock_reserve_view srv on pf.process_id <=> srv.process_id
