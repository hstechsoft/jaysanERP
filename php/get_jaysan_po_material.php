...
return $data;
 ...
 $sql = "SELECT
   ...
   concat('[',GROUP_CONCAT( JSON_OBJECT('material_id', jpm.po_material_id ,'qty',    jpm.qty,'batch_id' ,jpm.batch_id,'material_rate',jpm.material_rate,'discount',jpm.disc,'is_approved' ,  jpm.is_approved, 'due_on',jpm.due_on ,'material_name',(select parts_tbl.part_name from parts_tbl where parts_tbl.part_id = jpm.po_material_id ),'original_qty',(select mrf_batch.batch_qty from mrf_batch where mrf_batch.batch_id = jpm.batch_id )) SEPARATOR ','),']' )as materials_list
