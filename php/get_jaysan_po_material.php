...
return $data;
 ...
 $sql = "SELECT
   ...
   concat('[',GROUP_CONCAT( JSON_OBJECT('po_material_id', jpm.jaysan_po_material_id ,'material_id', jpm.po_material_id ,'qty',    jpm.qty,'batch_id' ,jpm.batch_id,'material_rate',jpm.material_rate,'discount',jpm.disc,'is_approved' ,  jpm.is_approved, 'due_on',jpm.due_on ,'material_name',(select parts_tbl.part_name from parts_tbl where parts_tbl.part_id = jpm.po_material_id ),'uom',(SELECT uom FROM `mrf_purchase` WHERE mrf_id = (SELECT mrf_batch.mrf_id from mrf_batch WHERE mrf_batch.batch_id = jpm.batch_id) LIMIT 1),'gst',(select parts_tbl.gstrate from parts_tbl where parts_tbl.part_id = jpm.po_material_id ),'batch_date',(select  mrf_batch.batch_date from  mrf_batch where  mrf_batch.batch_id = jpm.batch_id ),'original_qty',ifnull((select mrf_batch.batch_qty from mrf_batch where mrf_batch.batch_id = jpm.batch_id ),jpm.qty)) SEPARATOR ','),']' )as materials_list
