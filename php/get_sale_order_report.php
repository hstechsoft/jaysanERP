...
dcf_final as (SELECT opid, JSON_ARRAYAGG(
     ...
     final as( SELECT emp_id,emp_name,sop_view.oid,sop_view.opid,sop_view.order_category,sop_view.customer_id,sop_view.sale_order_date,sop_view.order_no,sop_view.cus_name,sop_view.cus_phone,
       ...
       sales_spares as (SELECT oid,JSON_ARRAYAGG(JSON_OBJECT('spares_id',spares_id,'qno',qno,'remark',remark,'amount',amount)) as spares,dcf_no,dcf.sts from sale_order_spares
left join dcf on sale_order_spares.dcf_no = dcf.dcf_id
GROUP BY dcf_no,oid), 
spares_final as(SELECT oid,JSON_ARRAYAGG(JSON_OBJECT('dcf_no',dcf_no,'sts',sts,'spares',spares)) as spares from sales_spares GROUP by oid)
       select final.*,sale_order_payment_full.*,credit as paid,debit as total_payment,bal from final 
       left join sale_order_payment_full on final.oid = sale_order_payment_full.oid
 left join spares_final on final.oid = spares_final.oid
        where  $payment_query and $emp_id_query order by final.order_no desc;
