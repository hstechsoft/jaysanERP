...
$sql =  "SELECT soid as oid, dated,concat(customer.cus_name,'-',customer.cus_phone) as cus, employee.emp_name as emp, order_no,concat('<ul class=\"list-group\">',GROUP_CONCAT(concat(' <li class=\"list-group-item\"><div class=\"d-flex justify-content-center gap-2 p-1 \"><p class=\"small m-0 p-0\">',pr,'</p>  <p class=\"small  m-0 p-0\">',ma,'</p> <p class=\"small  m-0 p-0\">',tn,'</p> <p class=\"small m-0 p-0\">( ',st,' )</p> <p class=\"small fw-bold m-0 p-0\"> - ',ass_pro.pro_qty,'</p></div> </li>') SEPARATOR ''),'</ul>') as pro, count(so_opid)as rowspan1,rowspan,concat('<ul class=\"list-group\">',GROUP_CONCAT(concat(' <li class=\"list-group-item\">',details,'</li>')  separator ''),'</ul>')  as sts from(SELECT sof.oid as soid, sof.dated,sof.order_no, sof.customer_id,sof.emp_id,(SELECT jaysan_final_product.product_name from jaysan_final_product WHERE jaysan_final_product.product_id = jaysan_product_model.product_id) as pr,jaysan_product_model.model_name as ma,jaysan_model_type.type_name as tn,ass_pro.st,ass_pro.eo,ass_pro.ln,ass_pro.oid,ass_pro.so_opid,ifnull(sum(ass_pro.tot_ass_pro),0) as tot_ass_pro,ass_pro.pro_qty,
