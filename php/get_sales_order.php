...
if($approve_sts == 0){
  $sql =  "SELECT sod.*,concat('<ul class=\"list-group\">',GROUP_CONCAT(concat('<li class=\"list-group-item \"><span class=\"text-bg-secondary me-1\">',(SELECT jaysan_final_product.product_name from jaysan_final_product WHERE jaysan_final_product.product_id = jaysan_product_model.product_id),'</span>  <span class=\"text-bg-info me-1\">', jaysan_product_model.model_name,'</span> <span class=\"text-bg-success me-1\">',jaysan_model_type.type_name,'</span> <span class=\"text-bg-warning\"> Qty :',sod.required_qty , '</span> <p class=\"m-0 p-0\">',sod.sub_type,'</p> </li>' ) SEPARATOR ''),'</ul') as product,DATE_FORMAT(sod.dated, '%d-%m-%Y %h:%i %p') as so_date from (SELECT sales_order_product.type_id,sales_order_product.oid,sales_order_product.model_id,sales_order_product.sub_type,sales_order_product.required_qty,employee.emp_name,concat(customer.cus_name , '</br>' , customer.cus_phone)as customer,sales_order_form.first_payment_date as dated,sales_order_form.order_no,sales_order_form.order_category from sales_order_form  INNER join sales_order_product on sales_order_form.oid = sales_order_product.oid inner JOIN employee on employee.emp_id = sales_order_form.emp_id INNER join customer on sales_order_form.customer_id = customer.cus_id where sales_order_form.emp_id =  $emp_id and sales_order_form.approve_sts =  '0') as sod inner join jaysan_product_model on sod.model_id = jaysan_product_model.model_id inner join jaysan_model_type on sod.type_id = jaysan_model_type.mtid GROUP by sod.oid ORDER by sod.order_no asc";
}else{  
  $sql =  "SELECT sod.*,concat('<ul class=\"list-group\">',GROUP_CONCAT(concat('<li class=\"list-group-item \"><span class=\"text-bg-secondary me-1\">',(SELECT jaysan_final_product.product_name from jaysan_final_product WHERE jaysan_final_product.product_id = jaysan_product_model.product_id),'</span>  <span class=\"text-bg-info me-1\">', jaysan_product_model.model_name,'</span> <span class=\"text-bg-success me-1\">',jaysan_model_type.type_name,'</span> <span class=\"text-bg-warning\"> Qty :',sod.required_qty , '</span> <p class=\"m-0 p-0\">',sod.sub_type,'</p> </li>' ) SEPARATOR ''),'</ul') as product,DATE_FORMAT(sod.dated, '%d-%m-%Y %h:%i %p') as so_date from (SELECT sales_order_product.type_id,sales_order_product.oid,sales_order_product.model_id,sales_order_product.sub_type,sales_order_product.required_qty,employee.emp_name,concat(customer.cus_name , '</br>' , customer.cus_phone)as customer,sales_order_form.first_payment_date as dated,sales_order_form.order_no,sales_order_form.order_category from sales_order_form  INNER join sales_order_product on sales_order_form.oid = sales_order_product.oid inner JOIN employee on employee.emp_id = sales_order_form.emp_id INNER join customer on sales_order_form.customer_id = customer.cus_id where sales_order_form.emp_id =  $emp_id and sales_order_form.approve_sts !=  '0') as sod inner join jaysan_product_model on sod.model_id = jaysan_product_model.model_id inner join jaysan_model_type on sod.type_id = jaysan_model_type.mtid GROUP by sod.oid ORDER by sod.order_no asc";
}
