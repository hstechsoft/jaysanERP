...
$sql = "SELECT COUNT(work_attachment.work_id)as attach, work_description,work_date,work_type,work.emp_id,work.work_id,emp_name FROM work INNER JOIN employee ON employee.emp_id = work.work_created_by LEFT join work_attachment  on work.work_id  = work_attachment.work_id where work.emp_id = $emp_id and work.work_assign_status ='pending' GROUP by work.work_id ORDER BY work_date";
