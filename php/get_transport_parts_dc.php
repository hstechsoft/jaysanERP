...
$sql = "with transport as(SELECT tdc.des_godown, dg.creditor_name as des_godown_name, sr.reserve_type,sr.reserve_type_id,js.godown,js.dep,js.sec,creditors.creditor_name,dep.dep_name,ds.sec_name, JSON_ARRAYAGG(JSON_OBJECT('stock_reserve_id',sr.stock_reserve_id,'part_id',js.part_id,'process_id',js.process_id,'part_name',ifnull(pt.part_name, CONCAT('semi finished part (', jp.process_name, ')')) ,'process_name', jp.process_name,'qty',sr.reserve_qty,'stock_id',sr.stock_id,'godown',js.godown)) as parts from  transport_dc tdc
...
left join creditors  dg on tdc.des_godown = dg.creditor_id
 ...
 WHERE tdc.source_godown = $godown and tdc.sts = 'create' and tdc.current_transport is NULL
group by tdc.des_godown,js.godown,js.dep,js.sec)
