...
<option value='Waiting'>Waiting</option>
 ...
 </div><div></div><div> <button value='\",opid,\"' class=' \",bal_cat, \" pro_btn btn btn-primary btn-sm'>Add</button></div> </div>  </li>\") SEPARATOR '') as product_list from(SELECT (SELECT jaysan_final_product.product_name from jaysan_final_product WHERE jaysan_final_product.product_id = (SELECT  jaysan_product_model.product_id from jaysan_product_model WHERE jaysan_product_model.model_id = sales_order_product.model_id)) as product, (SELECT jaysan_model_type.type_name from jaysan_model_type WHERE jaysan_model_type.mtid = sales_order_product.type_id)as type,(SELECT jaysan_product_model.model_name from jaysan_product_model WHERE jaysan_product_model.model_id = sales_order_product.model_id)as model, employee.emp_name,customer.cus_name,customer.cus_phone, sales_order_product.* ,concat(sales_order_form.order_no, if(sales_order_form.order_category = 'Sales', concat('<span class=\"ms-1 badge bg-secondary\">','S','</span>'), concat('<span class=\"ms-1 badge bg-info\">','R','</span>'))) as order_no,(SELECT ifnull(sum(assign_product.qty),0 )from assign_product WHERE assign_product.opid = sales_order_product.opid) as assign_qty , (SELECT IF(sales_order_product.required_qty  > assign_qty, '', 'disabled')) AS bal_cat , (SELECT sales_order_product.required_qty- assign_qty) as bal_qty,DATE_FORMAT(sales_order_form.dated, '%d-%m-%Y %h:%i %p') as date_f from sales_order_form INNER JOIN sales_order_product on sales_order_form.oid = sales_order_product.oid INNER join customer on sales_order_form.customer_id = customer.cus_id INNER join employee on sales_order_form.emp_id =  employee.emp_id  where sales_order_form.approve_sts != 1)  as g WHERE oid in (SELECT DISTINCT oid from(SELECT DISTINCT  jaysan_payment.oid from jaysan_payment WHERE jaysan_payment.sts = 'approved'

UNION all SELECT DISTINCT sale_payment_advance.oid from sale_payment_advance WHERE sale_payment_advance.payment_id is NULL) as oid_array)  group by oid;";
