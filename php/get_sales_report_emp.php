<?php
 ...
 $order_cat = test_input($_GET['order_cat']);
...
if($approve_sts == "'0'")
  ...
  INNER join sale_order_payment_view sopv  on sales_pro.oid = sopv.oid
  left join  sale_order_payment_full on sales_pro.oid = sale_order_payment_full.oid WHERE sof.order_category =  $order_cat and sales_pro.emp_id =  $emp_id  
   order by sof.order_no ASC";
...
INNER join sales_product_view spv  on  sop.opid = spv.opid 
 INNER join sales_order_sts_view sosv  on  sop.opid = sosv.opid 
INNER join sales_order_form sof  on  sof.oid = sop.oid WHERE sof.approve_sts > 0 GROUP by oid ) as sales_pro 
INNER join employee emp on sales_pro.emp_id = emp.emp_id
INNER join customer cus on sales_pro.customer_id = cus.cus_id
INNER join sales_order_form sof  on  sales_pro.oid = sof.oid 
INNER join sale_order_payment_view sopv  on sales_pro.oid = sopv.oid
left join  sale_order_payment_full on sales_pro.oid = sale_order_payment_full.oid
 WHERE sof.order_category =  $order_cat and sales_pro.emp_id =  $emp_id     order by sof.order_no ASC";
