...
$sql = "select ifnull(stock_view.reserve_qty,0) as reserved_qty,creditors.creditor_name as godown_name, department.dep_name, dep_section.sec_name  from  stock_view 

 left join creditors on creditors.creditor_id <=> stock_view.godown 
left join department on stock_view.dep <=> department.dep_id
left join dep_section on stock_view.sec <=> dep_section.dep_sec_id 
 where $part_query and $process_query  and stock_view.reserve_type = 'job_work_order'";
