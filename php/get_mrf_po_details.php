...
WHERE
    mrf_batch.sts = 'create' AND mrf_batch.batch_date <= CURRENT_DATE() and mrf_batch.batch_id not in (select batch_id from jaysan_po_material where batch_id not in(SELECT if(ifnull(sum(qty),0)=mrf_batch.qty,batch_id,'') from jaysan_po_material WHERE batch_id = 28 GROUP by batch_id)   group by batch_id);
SQL;
// $sql .= "SELECT
//     mrf_purchase.mrf_id,
//     mrf_purchase.mrf_purchase_id,
