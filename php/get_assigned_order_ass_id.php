...
customer.cus_phone,jaysan_model_type.type_name as type,line_no as qr_no,production_id FROM
( SELECT sales_order_form.*,hs.type_id as hstype_id,hs.model_id,hs.sub_type,hs.ass_date,hs.ass_id,line_no,production_id
    FROM
        (
        SELECT
            sales_order_product.*,
            machine_production_taken.line_no,
            machine_production_taken.production_id,
        ...
        inner join machine_production_taken on machine_production_taken.ass_id = assign_product.ass_id where assign_type = "production" and dcf_id = 0 and machine_production_taken.ass_id = $ass_id
    ) AS hs
