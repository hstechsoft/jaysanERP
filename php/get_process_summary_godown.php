...
SELECT  if(creditor_id > 0,if(department.dep_id > 0,if(dep_section.dep_sec_id > 0, CONCAT(creditor_name, ' - ', dep_name, ' - ', sec_name), CONCAT(creditor_name, ' - ', dep_name)), creditor_name), 'N/A') as production_place, creditor_name as godown_name,dep_name,sec_name,creditor_id,department.dep_id,dep_section.dep_sec_id,sum(ifnull(cost,0) ) * production_qty as total_cost, sum(ifnull(min_time,0)   ) * production_qty as total_min_time, sum(ifnull(max_time,0)) *production_qty as total_max_time, JSON_ARRAYAGG(JSON_OBJECT('process_available_id', process_available_id, 'production_qty', production_qty, 'process', process, 'process_name', process_name, 'output_part_name', output_part_name, 'input_details', input_details, 'final_part_name', final_part_name, 'final_part', final_part, 'level', level, 'process_title', process_title, 'cost', cost, 'min_time', min_time, 'max_time', max_time)) as process_details FROM process_details 
