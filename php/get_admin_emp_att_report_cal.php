...
END AS Att_time_formatted, CASE
    WHEN total_time = 0 AND partial_count > 0 THEN 'Pa'
    WHEN total_time = 0 AND partial_count = 0 THEN 'Ab'
      WHEN total_time > 0  THEN 'Pr'
    ELSE 'Ab'
...
SELECT 
    ...
    ifnull( (SELECT  concat(  IF(att_req.leave_type = 'leave', 'L', 'T'),'-',IF(att_req.status = 'pending', 'P', 'A')) as sts
FROM att_req 
WHERE emp_id =  $emp_id
