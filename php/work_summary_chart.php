...
$sql = "SELECT IFNULL(table1.count,0)as count,date_seq.date as date from  (select count(*) as count, FROM_UNIXTIME(his_date/1000 ,'%Y-%m-%d') as date FROM `work_history`  as table1  WHERE his_date BETWEEN $start_date and $end_date and emp_id =  $emp_id GROUP by date ORDER BY date DESC) as table1 right join (select a.Date 
from (
