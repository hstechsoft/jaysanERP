...
left join department dep on js.dep = dep.dep_id 
 ...
 WHERE tdc.current_transport = $transport_godown and tdc.sts <> 'create' and tdc.sts = 'transport' 
group by sr.reserve_type,sr.reserve_type_id,js.godown,js.dep,js.sec)

SELECT reserve_type,reserve_type_id,dc_no,bill_to,ship_to,JSON_ARRAYAGG(JSON_OBJECT('godown',godown,'dep',dep,'sec',sec,'creditor_name',creditor_name,'dep_name',dep_name,'sec_name',sec_name,'parts',parts)) as parts from transport
