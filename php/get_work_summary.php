...
foreach ($stcok_zero_array as $zero_stock)
{
    ...
    $sr_part_id = sql_nullable($zero_stock['part_id']);
    $sr_previous_process_id = sql_nullable($pre_process_id);
    

    $sql_insert_stock_requset = "INSERT  INTO stock_request (part_id, previous_process_id, qty, godown, dep, sec, emp_id,updated_at) VALUES ( $sr_part_id,  $sr_previous_process_id,   {$zero_stock['required_qty']} - {$zero_stock['available_qty']} , $godown_id, $dep_id, $sec_id, $emp_id, NOW()) on duplicate key update updated_at = NOW(), qty = qty + (  {$zero_stock['required_qty']} - {$zero_stock['available_qty']})";
